...
public class BotonesTablaCompras {
    ...
    public void  manejarEventoTabla(MouseEvent evt) {
        ...
        if (fila >= 0 && columna >= 0) {
            ...
            if (nombre.equals("b_guardar")) {
                boolean exito = guardarCambiosEnBD(fila, columna);
                if (exito) {
    ...
    private void cargarDatosEnFormulario(int fila, int columna) {
        ...
        JButton btnGuardarFila = new JButton("Guardar");
        btnGuardarFila.setName("b_guardar");
        tblCompras.setValueAt(btnGuardarFila, fila, columna);
        tblCompras.repaint();
    ...
    private boolean guardarCambiosEnBD(int idCompra, int fila) {
