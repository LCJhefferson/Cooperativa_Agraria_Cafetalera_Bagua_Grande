...
public class RealizarCompras extends javax.swing.JInternalFrame {
    ...
    private RealizarCompras() {
        ...
        // Eliminar el borde y la barra de título si se desea
    ...
    /**
 * Verifica la longitud del texto y dispara la búsqueda.
 */

/**
 * Verifica la longitud del texto y dispara la búsqueda.
 * Maneja automáticamente DNI (8) y RUC (11).
 */
private void verificarLongitudYBuscar() {
    String dni = TxtDniSocio.getText().trim();
    int longitud = dni.length();
    
    // 1. Longitudes válidas (DNI=8, RUC=11)
    final int LONGITUD_DNI = 8;
    final int LONGITUD_RUC = 11;
    
    // Verificar si la longitud actual es un documento válido
    if (longitud == LONGITUD_DNI || longitud == LONGITUD_RUC) { 
        // Si el largo es 8 o 11, buscamos
        buscarSocioPorDNI(dni); 
        
    } else if (longitud == 0) {
        // 2. Campo vacío: limpiar labels
        LblNombreSocio.setText("Nombre: ");
        socioIdEncontrado = 0;
        
    } else {
        // 3. Longitud inválida (ej. 9, 10, o más de 11): limpiar y mostrar mensaje de espera
        LblNombreSocio.setText("Nombre: (Documento inválido)");
        socioIdEncontrado = 0;
    }
}
    ...
    private void initComponents() {
        ...
        jPanel2Layout.setVerticalGroup(
            ...
            .addGroup(jPanel2Layout.createSequentialGroup()
                ...
                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.UNRELATED)
                .addGroup(jPanel2Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    .addComponent(LblPrecioTotal, javax.swing.GroupLayout.Alignment.TRAILING, javax.swing.GroupLayout.PREFERRED_SIZE, 27, javax.swing.GroupLayout.PREFERRED_SIZE)
                    .addGroup(jPanel2Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.BASELINE)
                        .addComponent(jLabel5)
...
eliminarCompra();        // TODO add your handling code here:
    ...
    try {
        if (Desktop.isDesktopSupported() && Desktop.getDesktop().isSupported(Desktop.Action.BROWSE)) {
            ...
            // Si el sistema no soporta la acción de navegar
...
private void buscarSocioPorDNI(String dni) {
    ...
    try {
        ...
        // 🚨 Cierre de recursos completo y seguro
        try {
            if (rs != null) rs.close();
            if (pst != null) pst.close();
            if (cn != null) cn.close(); 
 ...
 private String generarGuiaIngreso(Connection cn, int idSocio) throws SQLException {
    ...
    String iniciales = obtenerInicialesSocio(cn, idSocio); // Llamada al nuevo método
    
    // Obtener fecha (formato DíaMesAño, ej: 121225)
    ...
    // ... (El resto del método permanece igual, pero cambia el SQL)
    ...
    try {
        // La consulta SQL busca la última guía que empieza con el nuevo prefijo
        String sql = "SELECT guia_ingreso FROM compra WHERE guia_ingreso LIKE CONCAT(?, '-%') ORDER BY id DESC LIMIT 1";
        
        pst = cn.prepareStatement(sql);
        // Usamos el prefijo (ej: JP-121225) como base de búsqueda
        pst.setString(1, prefijoGuia); 
        rs = pst.executeQuery();

        if (rs.next()) {
            guiaExistente = rs.getString("guia_ingreso");
            // Extraer el correlativo (los últimos 3 o 4 caracteres después del guion final)
            // Asumo que la guía es: JP-121225-001
    ...
    private void limpiarCamposCompra() {
        ...
        // Reiniciar el ComboBox a la primera opción ("--- Seleccione Producto ---")
 ...
 public void registrarNuevaCompra() {
    ...
    // Asegúrese de que este es el nombre de su clase (ItemProducto o ITProducto)
    ...
    // Eliminada: int cantidad = TxtCantidad.getText(int).trim() <<< LÍNEA INCORRECTA
    ...
    try {
        // *****************************************************************
        // ** CONVERSIÓN DE STRING A DOUBLE (DEBE ESTAR DENTRO DEL TRY) **
        // *****************************************************************
        ...
        // *****************************************************************
        // ** USO DE LAS VARIABLES DOUBLE CONVERTIDAS (CORRECCIÓN CRÍTICA) **
        // *****************************************************************
        ...
        JOptionPane.showMessageDialog(null, "¡Compra registrada! Guía N°: " + guiaIngresoGenerada, "Éxito", JOptionPane.INFORMATION_MESSAGE);
        limpiarCamposCompra(); 
        cargarTablaCompras();
    ...
    try {
        ...
        if (rs.next()) {
            ...
            if (nombreCompleto.isEmpty()) {
                return "XX"; // Valor por defecto si el nombre está vacío
            }

            // Lógica para tomar la primera letra de cada palabra
...
public void cargarTablaCompras() {
    ...
    String[] titulos = {"ID", "Guía", "Fecha", "Socio", "Producto", "Cantidad","Rendimiento","Humedad", "Precio base","Total"};
DefaultTableModel modelo = new DefaultTableModel(null, titulos) {
    ...
    try {
        ...
        // Consulta SQL para obtener las últimas compras con nombres
...
String sql = "SELECT " +
        ...
        // 3. Iterar sobre los resultados y añadir filas al modelo
        
// Luego en el while, cambia el orden:
        while (rs.next()) {
            Object[] fila = new Object[10];
            fila[0] = rs.getInt("c.id");                    // ← ID oculto
        ...
        try {
            if (rs != null) rs.close();
            if (pst != null) pst.close();
            // Cerrar la conexión aquí si no se usa AutoCommit=false, sino cerrarla en el método que la abre.
            // Si su método Conexion.getConexion() abre una nueva, debe cerrarse aquí:
            if (cn != null) cn.close(); 
        } catch (SQLException ex) {
            // Manejo de error al cerrar
        }
...
private void calcularPrecioTotal() {
    try {
        ...
        // Si algún campo está vacío, mostramos 0.00 y terminamos
...
private void deshabilitarModoEdicion() {
    ...
    // ** NUEVO: HABILITAR el botón Modificar **
    BtnModificar.setEnabled(true); 
    
    TxtDniSocio.setEnabled(true);
    LblNombreSocio.setEnabled(false);// Habilitar búsqueda de socio
...
private void cargarDetallesCompraPorId(int idCompra) {
    ...
    try {
        ...
        if (rs.next()) {
            ...
            // CARGAR SOCIO Y PRODUCTO USANDO LA MISMA CONEXIÓN
            cargarSocioParaEdicion(idSocio, cn);      // ← pasa cn
            seleccionarProductoPorId(idProducto, cn); // ← pasa cn
            ...
            calcularPrecioTotal();
            habilitarModoEdicion(); // ← cambia el botón a "GUARDAR CAMBIOS"
        }
        ...
        try {
            if (rs != null) rs.close();
            if (pst != null) pst.close();
            if (cn != null) cn.close();  // ← CIERRE DE CONEXIÓN
        } catch (SQLException ex) {
            ex.printStackTrace();
        }
    }
...
private void eliminarCompra() {
    ...
    int opcion = JOptionPane.showConfirmDialog(this,
        "<html><b>¿Eliminar permanentemente esta compra?</b><br>" +
        "Guía: <b>" + guia + "</b><br>" +
    ...
    // === ELIMINAR CON CONEXIÓN NUEVA Y FRESCA ===
    Connection cn = null;
    PreparedStatement pst = null;

    try {
        // ¡SIEMPRE abre una conexión NUEVA aquí!
        cn = Conexion.getConexion();  // ← conexión fresca
        ...
        if (filas > 0) {
            ...
            registrarEnBitacora("compra", "DELETE", 
                "Eliminada compra ID=" + idCompra + ", Guía=" + guia + 
                ", Usuario=" + Sesion.usuarioActual);

            cn.commit();

            JOptionPane.showMessageDialog(this, 
                "¡Compra eliminada correctamente!", "Éxito", JOptionPane.INFORMATION_MESSAGE);
